...
#ifdef USE_SOS_MODULE
        // Con il modulo SOS il triple-click deve funzionare anche con lo schermo
        userConfig.triplePress = INPUT_BROKER_USER_PRESS; // valore placeholder, gestito in ButtonThread
#endif
        UserButtonThread->initButton(userConfig);
#ifdef USE_SOS_MODULE
        // Corregge il click timing per permettere triple-click fluido
        // (senza questo, con screen=true, setClickMs(20) rende impossibile il triple-click)
        UserButtonThread->userButton.setClickMs(SOS_CLICK_MS);
#endif
    } else {
...
#ifdef USE_SOS_MODULE
        // Stesso fix del path con-schermo: assicura che il click timeout
        // sia abbastanza lungo per il triple-click
        UserButtonThread->userButton.setClickMs(SOS_CLICK_MS);
#endif
    } // end else (no screen)
#endif
...
#ifdef USE_SOS_MODULE
    // Pulsante esterno SOS su GPIO SOS_EXT_BUTTON_PIN (default GPIO8)
    // Gestito con polling in SOSModule::runOnce() per evitare conflitti con InputBroker
    pinMode(SOS_EXT_BUTTON_PIN, INPUT_PULLUP);
    LOG_INFO("SOS: pulsante esterno su GPIO%d inizializzato (pull-up)", SOS_EXT_BUTTON_PIN);
#endif
}
